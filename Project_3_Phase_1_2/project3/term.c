...
// semaphore for each node insertion
sem_t sem_queue;
// semaphore for each queue nodes 
sem_t sem_mutex;
...
{ TermQnode *node;
  ...
  { node = termQhead;
    ...
    sem_post(&sem_queue);
    sem_post(&sem_mutex);
    //if(termQhead == NULL) sem_wait(&sem_queue);
  }
}
...
{ int ret;
  ...
  if(ret < 0) printf("Error occured while creating terminal thread\n");
  else printf ("Terminal thread created ... \n");
