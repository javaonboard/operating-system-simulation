...
{ EndIOnode *node;
  ...
  sem_wait (&preader);
  readercount++;
  if(readercount==1)sem_wait(&pwriter);
  sem_post(&preader);
  printf("priotrize insert IO\n");
  ...
  sem_wait(&preader);
  readercount--;
  if(readercount==0)sem_post(&pwriter);    
  sem_post (&preader);
}
...
void initialize_process_manager ()
{
  ...
  init_idle_process ();
  sem_init (&preader, 0, 1);
  sem_init (&pwriter, 0, 1);
}
